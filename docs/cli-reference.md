...
- With `--actions`, workflow setting panels expose guarded editors for allow-listed workflow knobs, including the compile-gate command, full-suite command and limits, project-memory tuning, and auto-backup settings. The editor supports preview and execution modes, requires the exact confirmation phrase `APPLY GARDA SETTING`, and runs the existing audited `garda workflow set` command path with `--operator-confirmed yes`; it does not write `workflow-config.json` directly.
