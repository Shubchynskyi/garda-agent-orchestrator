...
- Optional Markdown working plans live at `garda-agent-orchestrator/runtime/plans/<task-id>.md`. Agents may read them as human-readable executor guidance, but they are not passed as `--plan-path`, are not schema-enforced, and missing files are not a gate, review, or completion blocker.
