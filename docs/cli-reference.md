...
- The default scope budget guard is enabled for `strict`, uses `BLOCK_FOR_SPLIT`, and limits large tasks before expensive gates with `max_files=12`, `max_changed_lines=1200`, `max_required_reviews=5`, and `max_review_tokens=50000`.
- `max_required_reviews` means distinct review types required by the current preflight. Disabled review capabilities do not become required just because matching files changed; this is not the number of reviewers, completed review attempts, or review cycles.
