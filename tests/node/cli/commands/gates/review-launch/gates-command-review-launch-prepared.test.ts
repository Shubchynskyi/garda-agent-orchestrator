...
describe('cli/commands/gates review launch prepared metadata', () => {
    it('prepare-reviewer-launch writes current prepared launch metadata without attesting invocation', async () => {
        ...
        assert.ok(capturedLogs.some((line) => line.includes('StandbyResumeState: first_class_resumable_not_review_evidence')));
        assert.ok(capturedLogs.some((line) => line.includes('StandbyResumeInstruction: If the delegated reviewer session already reported standby completion')));
