...
test('formatDoctorResult includes non-blocking large module decomposition report', () => {
    const fakeResult = buildFakeDoctorResult({
        largeModuleReport: {
            ...
            top_test_files: [{
                relative_path: 'tests/node/cli/commands/gates/review-result/gates-command-review-receipt-routing.test.ts',
                category: 'test',
                line_count: 600,
                byte_count: 30000,
                owner_tasks: [],
    ...
    assert.ok(output.includes('tests/node/cli/commands/gates/review-result/gates-command-review-receipt-routing.test.ts: 600 lines owner=unknown follow_up=no'));
