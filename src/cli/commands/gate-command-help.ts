...
function buildReviewGateHelpEntries(
    ...
    return {
        ...
        'complete-reviewer-launch': {
            ...createSingleUsageEntry(
                'Complete reviewer launch metadata after a real delegated reviewer has been launched; fills post-launch fields without re-writing prepared hashes. Launch a real subagent using built-in tools with the exact CopyPasteReviewerLaunchPrompt or ReviewerLaunchArtifactPath; if for some reason that is impossible right now, you must stop and report this to the user; this is expected behavior in this repository.',
                `${cliPrefix} gate complete-reviewer-launch --task-id "${TASK_ID_PLACEHOLDER}" --review-type "<review-type>" --review-context-path "${buildBundleRelativePath(bundleName, `runtime/reviews/${TASK_ID_PLACEHOLDER}-<review-type>-review-context.json`)}" --reviewer-execution-mode "delegated_subagent" --reviewer-identity "<agent:...>" --reviewer-launch-artifact-path "${buildBundleRelativePath(bundleName, `runtime/tmp/reviews/${TASK_ID_PLACEHOLDER}/<review-type>/reviewer-launch.json`)}" --provider-invocation-id "<actual-invocation-id>" --attestation-source "<provider-source>" --launch-input-mode "launch_artifact_path" --launch-input-artifact-path "${buildBundleRelativePath(bundleName, `runtime/tmp/reviews/${TASK_ID_PLACEHOLDER}/<review-type>/reviewer-launch.json`)}" --launch-input-sha256 "<prepared-launch-artifact-sha256>" --fork-context false --record-invocation --repo-root "."`,
                true
            )
        },
