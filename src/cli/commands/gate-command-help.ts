...
function buildReviewGateHelpEntries(
    ...
    return {
        ...
        'prepare-reviewer-launch': {
            ...createSingleUsageEntry(
                'Prepare task-owned delegated reviewer launch metadata from the current routing event and review context; this is not completed invocation evidence. After preparation, launch one clean-context delegated reviewer with the exact CopyPasteReviewerLaunchPrompt or ReviewerLaunchInputArtifactPath, then record reviewer delegation start before the reviewer returns. Provider fallback only: if a provider truly requires a session before launch input exists, use standby/resume for that same session after prepare-reviewer-launch; standby or STANDBY_READY completion before launch input delivery is handshake noise, not review evidence.',
        ...
        'record-reviewer-delegation-started': {
            ...createSingleUsageEntry(
                ...
                `${cliPrefix} gate record-reviewer-delegation-started --task-id "${TASK_ID_PLACEHOLDER}" --review-type "<review-type>" --review-context-path "${buildBundleRelativePath(bundleName, `runtime/reviews/${TASK_ID_PLACEHOLDER}-<review-type>-review-context.json`)}" --reviewer-execution-mode "delegated_subagent" --reviewer-identity "<agent:...>" --reviewer-launch-artifact-path "${buildBundleRelativePath(bundleName, `runtime/tmp/reviews/${TASK_ID_PLACEHOLDER}/<review-type>/reviewer-launch.json`)}" --provider-invocation-id "<provider-owned invocation id from delegated reviewer launch result>" --attestation-source "<provider-owned attestation source from delegated reviewer launch result>" --launch-input-mode "launch_artifact_path" --launch-input-artifact-path "${buildBundleRelativePath(bundleName, `runtime/tmp/reviews/${TASK_ID_PLACEHOLDER}/<review-type>/reviewer-launch-input.json`)}" --launch-input-sha256 "<ReviewerLaunchInputArtifactSha256>" --fork-context false --repo-root "."`,
                true
            )
        },
