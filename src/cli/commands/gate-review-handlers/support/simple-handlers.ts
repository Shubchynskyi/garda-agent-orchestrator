...
export async function handleRequiredReviewsCheck(gateArgv: string[]): Promise<void> {
    const defs = {
        ...
        '--review-authorship-attestation-json': { key: 'reviewAuthorshipAttestationJson', type: 'string' },
...
export async function handleDocImpactGate(gateArgv: string[]): Promise<void> {
    const defs = {
        ...
        '--project-memory-update-not-needed': { key: 'projectMemoryUpdateNotNeeded', type: 'boolean' },
