...
export function runCleanup(options: CleanupOptions): CleanupResult {
    ...
    if (!dryRun && policy.maxAggregateLines > 0) {
        try {
            aggregateRetention = pruneAggregateLogLocked(path.join(runtimeDir, 'task-events'), policy.maxAggregateLines);
        } catch {
            // best-effort
        }
    }
...
export function runGc(options: GcOptions): GcResult {
    ...
    if (shouldApplyStoragePolicy && confirm) {
        ...
        storagePolicyResult = applyStoragePolicy(path.join(runtimeDir, 'reviews'), storagePolicy, activeIds);
    }
