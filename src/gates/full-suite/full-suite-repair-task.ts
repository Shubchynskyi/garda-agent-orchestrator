...
const WIP_MANIFEST_SCHEMA_VERSION = 2;
...
interface MaterializeFullSuiteRepairTaskParams {
    repoRoot: string;
    taskId: string;
...
interface RestoreFullSuiteRepairWipParams {
    repoRoot: string;
    taskId: string;
    fullSuiteArtifactPath: string;
    manifestPath: string;
...
interface PatchPathRecord {
    paths: string[];
}

interface RestorePatchSnapshots {
    staged: Buffer | null;
    unstaged: Buffer | null;
}

interface RestoreMutationInspection {
    violations: string[];
...
function validatePhysicalRepoContainment(repoRoot: string, absolutePath: string, label: string): string[] {
    const root = path.resolve(repoRoot);
...
function assertSingleLinkIdentity(identity: fs.Stats, label: string): void {
    if (!identity.isFile()) {
...
function restoreContainedUntrackedFile(params: {
    repoRoot: string;
...
function restoreContainedUntrackedBytes(params: {
    repoRoot: string;
    entry: CapturedUntrackedFileEvidence;
    content: Buffer;
}): void {
...
function runGitStatus(repoRoot: string, args: string[], input?: Buffer): {
    status: number;
    stdout: string;
    stderr: string;
} {
...
function runGitWithInput(repoRoot: string, args: string[], input: Buffer): string {
...
function validateManifestRelativePath(repoRoot: string, value: unknown, label: string): string[] {
...
function parseRepairWipManifest(repoRoot: string, manifestPath: string, value: unknown): {
