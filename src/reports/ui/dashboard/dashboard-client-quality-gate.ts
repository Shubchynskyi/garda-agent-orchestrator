...
function renderQualityGateRuleRow(rule, disabled) {
  ...
  return '<tr data-optional-rule-id="' + ruleId + '" data-optional-rule-source="' + safe(ruleSource) + '">'
    ...
    + '<td class="quality-gate-rule-actions"><div class="setting-buttons"><button type="button" data-quality-gate-rule-action="upsert" data-quality-gate-rule-id="' + ruleId + '" data-quality-gate-rule-source="' + safe(ruleSource) + '"' + (disabled ? ' disabled' : '') + '>' + safe(disabled ? t('saveDisabled') : actionLabel) + '</button><button type="button" data-quality-gate-rule-action="delete" data-quality-gate-rule-id="' + ruleId + '" data-quality-gate-rule-source="' + safe(ruleSource) + '"' + deleteDisabledAttr + '>' + safe(t('removeOptionalCheckRule')) + '</button></div></td>'
    + '</tr>';
}
function renderQualityGateNewRuleRow(disabled) {
  ...
  return '<tr data-optional-rule-id="quality-gate-new"><td><input id="' + safe(newId) + '" type="text" placeholder="custom_rule_id"' + disabledAttr + '></td><td>' + safe(t('qualityGateSourceCustom')) + '</td><td>' + badge(t('qualityGateNewRule'), 'quality-gate-rule', 'quality-gate-rule-new') + '</td><td><input id="' + safe(newTitle) + '" type="text"' + disabledAttr + '></td><td><input id="' + safe(newPrompt) + '" type="text"' + disabledAttr + '></td><td><select id="' + safe(newEnabled) + '"' + disabledAttr + '><option value="true">' + safe(t('gardaSwitchStateOn')) + '</option><option value="false">' + safe(t('gardaSwitchStateOff')) + '</option></select></td><td><label class="quality-gate-scope-control"><input id="' + safe(newExcludeTestOnly) + '" type="checkbox"' + disabledAttr + '><span>' + safe('Exclude test-only') + '</span></label></td><td class="quality-gate-rule-actions"><div class="setting-buttons"><button type="button" data-quality-gate-rule-action="upsert" data-quality-gate-rule-id="quality-gate-new"' + (disabled ? ' disabled' : '') + '>' + safe(t('addOptionalCheckRule')) + '</button></div></td></tr>';
}
