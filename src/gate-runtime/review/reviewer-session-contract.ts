export const REVIEWER_FRESH_CONTEXT_LAUNCH_INSTRUCTION =
    'Spawn a new clean-context delegated reviewer for this review context using the provider-native/internal agent or subagent tool with the exact generated CopyPasteReviewerLaunchPrompt or reviewer-facing ReviewerLaunchInputArtifactPath from prepare-reviewer-launch, not ReviewerLaunchArtifactPath control metadata, not a shell command or hand-written artifact, and not a reconstructed prompt; do not create, reserve, hold, or complete a reviewer before launch input exists, and do not reuse an existing reviewer session. Codex should use multi_agent_v1.spawn_agent with fork_context=false, Claude should use fork_context=false when available, and other providers must use provider-equivalent isolated sub-agent or task launch. If the current provider session cannot launch a fresh delegated reviewer, stop and report that blocker instead of fabricating routing, launch, review, receipt, or telemetry evidence.';
...
export const REVIEWER_ONE_SHOT_LAUNCH_DEFAULT_INSTRUCTION =
    'After `prepare-reviewer-launch`, launch one clean-context delegated reviewer with the exact CopyPasteReviewerLaunchPrompt or reviewer-facing ReviewerLaunchInputArtifactPath, not ReviewerLaunchArtifactPath control metadata, then immediately run `record-reviewer-delegation-started` with the resolved provider reviewer identity, provider/controller invocation id, and exact launch-input evidence before the reviewer returns. Routing and preparation may use only the planned `agent:pending:<task-id>-<review-type>` identity; the resolved provider `agent:*` identity is not known until delegation start.';
