...
When this skill is selected for a mandatory `test` review, return the generated output template, not a free-form summary. Treat it as an immutable fill-in form: replace placeholder lines only. Preserve these headings exactly and in this order; never add, remove, rename, reorder, or nest headings:
1. `## Validation Notes` - concrete reviewed test files, behavior, boundaries, and verification evidence; required for PASS.
2. `## Findings by Severity` - canonical `None`, or active blocking test findings with file references using parser-supported inline/list formats.
