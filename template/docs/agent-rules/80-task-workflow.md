...
- Final user report order is mandatory: review integrity attestation -> concise implementation summary -> conventional-style `git commit -m "<type>(<scope>): <summary>"` suggestion -> `Do you want me to commit now? (yes/no)`.
- Final reports must start with a strict review integrity attestation: independent review completed yes/no/not required, fake/fallback/same-agent/legacy/missing/unverifiable artifacts observed yes/no, completion review-attested yes/no/not required, completion allowed yes/no, and the reason.
